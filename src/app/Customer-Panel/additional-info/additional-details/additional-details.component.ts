...
export class AdditionalDetailsComponent implements OnInit {
  ...
  pickupStatus:any
  constructor(private fb: FormBuilder,private deliveryDetailService:DeliveryDetailService,private sharedService:SharedService,private indexService:IndexService,private mergeService:MergeService) { }
  ...
  ngOnInit(): void {
    ...
    this.checkForm = this.fb.group({
      ...
      acceptReadFinancialServiceGuide: [{value: false, disabled: true}, Validators.required],
      emailForDocuments: ['john@gmail.com', [Validators.required, Validators.email]],
      isPickupRequired: [],
...
this.deliveryDetailService.sharedData$.subscribe(res=>{
  ...
  //  this.orderform=JSON.parse(localStorage.getItem('orderForm') || '');
   console.log("order form value",this.orderform);
     ...
     this.Calculateprice = this.getQuoteById.ishhiperFinalPrice-this.getQuoteById.ishipperCommission
    -this.getQuoteById.ishipperGST
  ...
  checkTermsConditions(value: string) {
    const currentStatus = this.checkForm.get(value)?.value;
    ...
    // Check if all checkboxes (except the last one) are selected
    this.updateReadFinancialServiceGuide();
    console.log("Updated form values", this.checkForm.value);
  }

  // Update the last checkbox's disabled state based on other checkboxes
  updateReadFinancialServiceGuide() {
    const allChecked = this.checkForm.get('acceptAttachment')?.value &&
    ...
    if (allChecked) {
      this.checkForm.get('acceptReadFinancialServiceGuide')?.enable();
    } else {
      this.checkForm.get('acceptReadFinancialServiceGuide')?.disable();
    }
